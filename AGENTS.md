...
| **Capture system parameter values for observability** | `#[system(inspect(a, b, return))]` selects at compile time; `InspectionPlugin` delivers - enable via `InspectionAPI`, listen via `Extends<InspectionSinkRegistry>` | `polaris_core_plugins/src/inspection.rs`, `polaris_system/src/param/inspect.rs` | [System - Parameter Inspection](docs/reference/system.md#parameter-inspection) |
| **Keep credentials out of captured values and telemetry export** | `RedactionRules` rules by binding name or type (covered values arrive `Redacted`, `Debug` never run) - `InspectionPlugin::with_redactions` at build, `InspectionAPI::add_redacted_param` / `add_redacted_type` at run time; `disable_listener(INSPECTION_TRACING_LISTENER)` cuts export alone | `polaris_core_plugins/src/inspection.rs` | [System - Parameter Inspection](docs/reference/system.md#parameter-inspection) |
