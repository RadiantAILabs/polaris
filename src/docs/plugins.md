...
| [`InspectionPlugin`](crate::plugins::InspectionPlugin) | Delivery for `#[system(inspect(..))]` parameter captures: the [`InspectionAPI`](crate::plugins::InspectionAPI) runtime switch (off by default; starting policy and added redactions can come from environment-variable specs the application names, via `with_policy_from_env` / `with_redactions_from_env`) with [`RedactionRules`](crate::plugins::RedactionRules) and checked typed-listener toggles, the [`InspectionSinkRegistry`](crate::plugins::InspectionSinkRegistry) listener sign-up sheet, and the shipped [`TracingInspectionSink`](crate::plugins::TracingInspectionSink) that emits records onto the per-step tracing span. |
